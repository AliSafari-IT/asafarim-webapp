...
export const Layout: React.FC<LayoutProps> = ({
  ...
  const sidebarStyle = {
    ...
    transition:
      "margin-left 0.3s ease, margin-right 0.3s ease, width 0.3s ease",
  };
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      if (
        isMobileView &&
        !document.documentElement.hasAttribute("sidebar-toggled")
      ) {
  ...
  return (
    <div
      className={`${styles.layout} ${styles[theme]} animate-fade-scale`}
    ...
    >
      ...
      <Footer theme={theme} style={sidebarStyle} />
    </div>
  );
};
